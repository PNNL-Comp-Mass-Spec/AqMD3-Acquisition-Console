		...
		server->register_handler([&](Server::ReceivedRequest req)
		{
			...
			if (command == "acquire frame")
			{
				if (req.payload.size() == 2)
				{
					...
					auto context = digitizer.configure_cst_zs1(digitizer.channel_1, 100, get<0>(tof_params), Digitizer::ZeroSuppressParameters(0, 200, 8, 0));
			...
			if (command == "acquire")
			{
				...
				auto context = digitizer.configure_cst_zs1(digitizer.channel_1, 100, record_size, Digitizer::ZeroSuppressParameters(0, 200, 8, 0));
				cout << "record size " << record_size << std::endl;

				auto dp = DataPublisher(server->get_publisher("tcp://*:6546"), record_size);
				threads.push_back(thread(publish_worker, ref(signal), ref(lock), ref(dataQueue), std::move(dp)));
...
static void digitizer_worker(std::condition_variable &sig, queue<AcquiredData>& resultsQueue, unique_ptr<StreamingContext> context)
{
	std::cout << "STARTING ACQUIRE" << std::endl;
	context->start();
	while(!should_exit)
	{
		...
		std::cout << "acquiring time: " << (t2 - t1).count() << std::endl;
	}
	context->stop();
}
...
{
	std::cout << "STARTING ACQUIRE FRAME" << endl;
	uint64_t total_acquire_time = 0;
	context->start();
	for(int i = 0; i < frame.frame_length; i += 100)
	{
		auto data = context->acquire(std::chrono::milliseconds(100));
		cout << "acquired " << data.stamps.size() << endl;
		resultsQueue.push(data);
		cout << "push " << i << endl;
		sig.notify_one();
	}
...
static void publish_worker(std::condition_variable &sig, std::mutex &lockable, queue<AcquiredData>& workQueue, DataPublisher&& publisher)
{
	...
	while (!should_exit)
	{
		...
		while (!workQueue.empty())
		{
			...
			process_index++;
			auto t2 = chrono::high_resolution_clock::now();
			std::cout << process_index << " :: processing time: " << (t2 - t1).count() << std::endl;
		}
	}
}
...
static void publish_worker_frame(std::condition_variable &sig, std::mutex &lockable, queue<AcquiredData>& workQueue, UimfFrame uimf)
{
	...
	while (!should_exit)
	{
		...
		while (!workQueue.empty())
		{
			...
			auto result = ae.process(1, 100 * process_index++);
			uimf.append_encoded_results(result);
		}
	}
